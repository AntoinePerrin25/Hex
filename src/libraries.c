...
bool reprise(partie_t *partie, const char *path, const char *nomsauvegarde)
{
    ...
    else
    {
        ...
        for (int i = 0; i < partie->taille; i++) {                      // pour chaque ligne et colonne, on recupere le nombre associé, dont on place
            for (int j = 0; j < partie->taille; j++) {                  // la valeur dans le plateau
            int caseplateau = ' ';                                         
...
bool gagnant(partie_t save) {
    ...
    for (int i = 0; i < save.taille+1; i++) { //toutes les cases du tableau contiennent des 0.
    ...
    if (save.joueur == 'X') // Condition de victoire de X : il gagne de gauche à droite
    {
        do
        {
            ...
            for (int i = 0; i < save.taille; i++) { // Si un pion X appartient au chemin et est sur la dernière colonne, la fonction retourne 1.
    ...
    if (save.joueur == 'O') // Condition de victoire de O : il gagne de de haut en bas
    {
        do
        {
            ...
            for (int j = 0; j < save.taille; j++) { // Si un pion O appartient au chemin et est sur la dernière ligne, la fonction retourne 1.
...
void regle(void)
{
    ...
    printf("          Au debut de la partie, un plateau vide separe deux joueurs.\n"
           ...
           "          Les deux joueurs sont representes par une forme : X et O.\n"
